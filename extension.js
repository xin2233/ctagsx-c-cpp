...
function findCTags(context, tag) {
    ...
    return ctagz.findCTagsBSearch(searchPath, tag)
        .then(result => {
            ...
            // 查找函数定义的tag (而不是声明)
            const definitionTags = options.filter(tag => tag.tagKind === 'function' && tag.kind === 'definition');

            // 如果找到定义，优先跳转到定义
            if (definitionTags.length) {
                return revealCTags(context, editor, definitionTags[0]);
            }

            // 如果没有找到定义，则跳转到声明或第一个匹配结果
